    ...
    private createSkills () {
        this.worldInfo.skills.forEach(skill => {
            ...
            this.skills.set(skill.name, new PlayerSkill(this.scene, skill, newSchema, this.traits, 
                this.player.upgradeManager().upgrades_for_skill(skill.id),
                this.player.consumableManager().consumables_for_skill(skill.id)))
        }, this)
    }
...
export class PlayerSkill extends Phaser.GameObjects.GameObject {
    constructor(scene : Phaser.Scene, skill : Chisel.Skill, schema : Skill, traits : AavegotchiTraits, upgrades : PlayerUpgrade[], consumables : Consumable[]) {
    ...
    private bindConsumables() {
        this.consumables.forEach(consumable => {
            consumable.on(Consumable.EXPIRED, this.generateValues, this)
            consumable.on(Consumable.ACTIVATED, this.generateValues, this)
        }, this);
    }
    ...
    private applyConsumables() {
        this.consumables.forEach(consumable => {
            ...
            if (consumable.active) {
                try {
                    scope.original = this.minimum
                    this.minimum = mathjs.evaluate(consumable.formula_for_skill(this.skill.id), scope)
                    scope.minimum = this.minimum

                    scope.original = this.maximum
                    this.maximum = mathjs.evaluate(consumable.formula_for_skill(this.skill.id), scope)
                    scope.maximum = this.maximum

                    scope.original = this.initial
                    this.initial = mathjs.evaluate(consumable.formula_for_skill(this.skill.id), scope)
