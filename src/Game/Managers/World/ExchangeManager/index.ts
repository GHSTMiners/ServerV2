...
const defaultPrices = new Map<string,number> ([
    ['DAI',  1],
    ['USDT', 1],
    ['MATIC',  1.2],
    ['GHST', 1.3],
        ...
        this.mainScene.worldInfo.crypto.forEach(crypto => {
            this.mainScene.room.presence.exists(`crypto_${crypto.id}_price`).then(exists => {
                if(exists) {
                    ...
                    needle('get', `https://polygon.api.0x.org/swap/v1/price?sellToken=DAI&buyToken=${crypto.wallet_address}&buyAmount=1000&skipValidation=true`).then(response => {
                        if(response.statusCode == 200) {
                            ...
                            let exchangeEntry : Schema.ExchangeEntry = new Schema.ExchangeEntry();
                            exchangeEntry.crypto_id = crypto.id;
                            defaultPrices.has(crypto.shortcode)? exchangeEntry.usd_value = defaultPrices.get(crypto.shortcode): exchangeEntry.usd_value = 1;
                            this.mainScene.worldSchema.exchange.set(crypto.id.toString(), exchangeEntry)  
                        }
                    }).catch(error => {
                        let exchangeEntry : Schema.ExchangeEntry = new Schema.ExchangeEntry();
